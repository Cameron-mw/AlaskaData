...
app.layout = html.Div([
    ...
    dcc.Graph(id='map'), # Display Map
    html.Div(id='text_display', children=['text_display']) # Text Beneath Map
])

@app.callback(
    [dash.dependencies.Output('map', 'figure'), # Map Output
    dash.dependencies.Output('text_display', 'children')], # Text Output
...
def update_figure(selected_category):
    if selected_category == 'Communities': # Default Map showing all Communities
        ...
        text_display = html.H2('All Communities'),\
            ...
            'There are many rural Alaskan communities are not connected to the Alaska Highway system.' \
        ...
        filtered_df = df[df[selected_category] == 1] # Only show communities that belong to the selected category
        ...
        filtered_df = df[df[selected_category] == 1]
        text_display = 'Text Error'
    ...
    return fig, text_display # Return the map and the text beneath
